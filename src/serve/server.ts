...
let buildLlmsText = (opts: ServeOptions) => [
  ...
  "4. Write outputs to state.jsonl (the only writable file).",
...
let buildAgentManifest = (tokenSpec: TokenSpec) => ({
  ...
  workspaceSync: {
    ...
    writablePaths: ["state.jsonl"],
    readOnlyPaths: ["workspace.json", "messages.jsonl", "AGENTS.md"],
  },
  workflows: [
    "Pull a workspace snapshot into an isolated local directory.",
    "Read messages.jsonl and write summaries, drafts, and tasks to state.jsonl.",
    "Push state.jsonl back to the server.",
