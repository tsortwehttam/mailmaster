...
import { parseNoteCli } from "../src/note/cli"
import { parseBriefingCli } from "../src/briefing/cli"
import { verboseLog } from "../src/Verbose"

let args = hideBin(process.argv)
let subcommands = new Set(["gmail", "slack", "teams", "whatsapp", "ingest", "watch", "corpus", "serve", "draft", "workspace", "note", "briefing", "help"])
...
let cli = yargs(args)
  ...
  .command("workspace", "Initialize and manage agent workspaces for message monitoring")
  .command("note", "Create and manage notes, action items, and decisions")
  .command("briefing", "Manage the briefing queue for agent-assisted message review")
  ...
  .command(
    ...
    y =>
      y
        .positional("platform", {
          type: "string",
          choices: ["gmail", "slack", "teams", "whatsapp", "ingest", "watch", "corpus", "serve", "draft", "workspace", "note", "briefing"] as const,
    ...
    async argv => {
      ...
      if (argv.platform === "note") return parseNoteCli(helpArgs, "msgmon note")
      if (argv.platform === "briefing") return parseBriefingCli(helpArgs, "msgmon briefing")
    },
  )
  ...
  .epilog(
    [
      ...
      "  workspace — Initialize and manage agent workspaces.",
      "  note      — Create and manage notes, action items, and decisions.",
      "  briefing  — Manage the briefing queue for agent-assisted review.",
      "",
      "Platforms:",
...
// ---------------------------------------------------------------------------
// Note — manage notes, action items, decisions
// ---------------------------------------------------------------------------

else if (!dispatched && command === "note") {
  parseNoteCli([...forwardedVerboseArgs, ...commandArgs], "msgmon note").catch(e => {
    console.error(e?.message ?? e)
    process.exit(1)
  })
}
...
// ---------------------------------------------------------------------------
// Briefing — manage the briefing queue
// ---------------------------------------------------------------------------

else if (!dispatched && command === "briefing") {
  parseBriefingCli([...forwardedVerboseArgs, ...commandArgs], "msgmon briefing").catch(e => {
    console.error(e?.message ?? e)
    process.exit(1)
  })
}
...
// ---------------------------------------------------------------------------
  ...
  } else if (subhelp === "note") {
    parseNoteCli([...forwardedVerboseArgs, "--help"], "msgmon note")
  } else if (subhelp === "briefing") {
    parseBriefingCli([...forwardedVerboseArgs, "--help"], "msgmon briefing")
  } else {
    cli.parseAsync().catch(e => {
      console.error(e?.message ?? e)
      process.exit(1)
